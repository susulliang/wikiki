...
function findBestExcerpt(text, query, tokens) {
...
function scoreResult(result, query, tokens) {
    const productName = result.productName.toLowerCase();
    const pageName = result.pageName?.toLowerCase() ?? '';
    const snippet = result.snippet.toLowerCase();
    const phrase = query.toLowerCase().trim();
    let score = 0;
    if (phrase && productName.includes(phrase))
        score += 80;
    if (phrase && pageName.includes(phrase))
        score += 55;
    if (phrase && snippet.includes(phrase))
        score += 30;
    for (const token of tokens) {
        if (productName.includes(token))
            score += 20;
        if (pageName.includes(token))
            score += 12;
...
export function searchProducts(products, query) {
    ...
    for (const product of products) {
        ...
        product.pages.forEach((page, pageIndex) => {
            ...
            const searchableText = normalizeForSearch([page.name, pageText].join(' '));
...
export function highlightSearchText(text, query) {
    ...
    return parts.map((part, index) => tokens.some((token) => part.toLowerCase() === token.toLowerCase()) ? (<mark key={`${part}-${index}`} className="rounded-sm bg-primary/20 px-1 text-foreground">
        {part}
      </mark>) : (part));
}
