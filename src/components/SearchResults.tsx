...
interface SearchResultsProps {
  ...
  query: string;
  onSelect: (bundleId: string, pageIndex?: number) => void;
}
    ...
    <div className="flex-1 overflow-auto space-y-4 p-6">
      ...
      <div className="space-y-3">
        {results.map((result, i) => (
          <motion.button
            key={`${result.bundleId}-${result.pageId ?? 'name'}-${i}`}
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3, delay: i * 0.04 }}
            onClick={() => {
              const bundlePageIdx = result.pageId
                ? undefined
                : 0;
          ...
          >
            <div className="mb-2 flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                <span className="font-medium truncate">{highlightText(result.bundleName, query)}</span>
                {result.pageName && (
                  <>
