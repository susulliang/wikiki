...
export default function MindmapView({ content }) {
    ...
    const styledData = useMemo(() => {
        if (!treeData)
            return null;
        const processNode = (node, level, colorIdx) => {
            ...
            return {
                ...
                children: node.children?.map((child, idx) => processNode(child, level + 1, level === 0 ? idx : colorIdx))
            };
        };
    ...
    return (<div className="w-full h-full bg-background relative">
