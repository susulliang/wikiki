...
import { Bold, Italic, Underline, Strikethrough, List, ListOrdered, Code, Code2, Link, Image as ImageIcon, Quote, Minus, } from 'lucide-react';
...
export default function RichTextEditor({ content, onChange, showToolbar = true, stickyTop = 0 }) {
    ...
    return (<div className="flex flex-col flex-1 relative">
      {/* Toolbar */}
      {showToolbar && (<div className="sticky z-20 flex flex-wrap items-center gap-0.5 border-b border-border/70 bg-background/55 backdrop-blur-xl supports-[backdrop-filter]:bg-background/40 px-2.5 py-0.5" style={{ top: stickyTop }}>
          <div className="flex items-center gap-1 mr-1">
            <Button variant={activeHeading === 'p' ? 'secondary' : 'ghost'} size="sm" className="h-6 px-2 text-[11px]" onClick={() => handleHeadingChange('p')} title="Paragraph">
              Paragraph
            </Button>
            <Button variant={activeHeading === 'h1' ? 'secondary' : 'ghost'} size="sm" className="h-6 px-2.5 text-[11px]" onClick={() => handleHeadingChange('h1')} title="Heading 1">
              H1
            </Button>
            <Button variant={activeHeading === 'h2' ? 'secondary' : 'ghost'} size="sm" className="h-6 px-2.5 text-[11px]" onClick={() => handleHeadingChange('h2')} title="Heading 2">
              H2
            </Button>
            <Button variant={activeHeading === 'h3' ? 'secondary' : 'ghost'} size="sm" className="h-6 px-2.5 text-[11px]" onClick={() => handleHeadingChange('h3')} title="Heading 3">
              H3
            </Button>
          </div>
          ...
          <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs" onClick={handleImageUpload} title="Upload Image">
      ...
      {/* Editor area */}
      <div ref={editorRef} contentEditable suppressContentEditableWarning className="prose prose-sm max-w-none dark:prose-invert min-h-max h-full flex-1 px-8 py-6 outline-none focus:outline-none" onInput={handleInput} onKeyDown={handleKeyDown} onPaste={handlePaste} onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
        }} onDrop={handleDrop} onCompositionStart={handleCompositionStart} onCompositionEnd={handleCompositionEnd} data-placeholder="Start typing..."/>
    </div>);
}
